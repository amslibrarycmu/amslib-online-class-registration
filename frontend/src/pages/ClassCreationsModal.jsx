...
const ClassCreationModal = ({
  ...
  const handleLanguageChange = (lang) => {
    let currentLang = (formData.language === undefined || formData.language === null) ? "TH" : formData.language;
    let newLang = currentLang;

    const isTH = currentLang.includes("TH");
    const isEN = currentLang.includes("EN");

    if (lang === "TH") {
      if (isTH) {
        newLang = isEN ? "EN" : ""; 
      } else {
        newLang = isEN ? "TH/EN" : "TH";
      }
    } else if (lang === "EN") {
      if (isEN) {
        newLang = isTH ? "TH" : ""; 
      } else {
        newLang = isTH ? "TH/EN" : "EN";
      }
    }
    dispatch({ type: "SET_FIELD", payload: { name: "language", value: newLang } });
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (formData.target_groups.length === 0) {
      alert("กรุณาเลือกสถานภาพของผู้เรียนอย่างน้อย 1 กลุ่ม");
      return;
    }
      ...
      <div className="bg-white p-4 sm:p-6 lg:p-8 rounded-lg shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <label className="block font-medium mr-3 flex-shrink-0">ภาษา</label>
            <div className="flex flex-wrap gap-x-4 gap-y-2">
              <label className="flex items-center space-x-2 cursor-pointer">
                <input
                  type="checkbox"
                  checked={formData.language && formData.language.includes('TH')}
              ...
              <label className="flex items-center space-x-2 cursor-pointer">
                <input
                  type="checkbox"
                  checked={formData.language && formData.language.includes('EN')}
